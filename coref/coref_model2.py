...
class CorefModel:  # pylint: disable=too-many-instance-attributes
    ...
    def run2(self, 
        ...
        #cluster_emb is the merged clusters
        words = cluster_emb
        top_rough_scores, top_indices = self.rough_scorer(words)
    ...
    def _clusterize(self, doc: Doc, scores: torch.Tensor, top_indices: torch.Tensor):
        ...
        coref_span_heads = torch.arange(0, len(scores))[not_dummy] # indices of the antecedents where ant >= 0,// words which have an antecednt
    ...
    def _clusterize2(self, merged_cluster_ids : torch.Tensor , scores: torch.Tensor, top_indices: torch.Tensor):
        antecedents = scores.argmax(dim=1) - 1
        
        # wherever antecedent == -1, it means that that mention is a singleton
        not_dummy = antecedents >= 0
        not_dummy = not_dummy.to(torch.device('cpu'))
        dummy = antecedents == -1
        dummy = dummy.to(torch.device('cpu'))
        coref_span_heads = torch.arange(0, len(scores))[not_dummy]
        singletons = torch.arange(0, len(antecedents))[dummy]
        
        antecedents = top_indices[coref_span_heads, antecedents[not_dummy]]
        

        nodes = [GraphNode(i) for i in range(merged_cluster_ids.shape[0])]
        for i, j in zip(coref_span_heads.tolist(), antecedents.tolist()):
            nodes[i].link(nodes[j])
            assert nodes[i] is not nodes[j]
        ...
        for x in singletons:
            if int(x) not in l:
                final_singletons.append(int(x))

        for node in final_singletons:
    ...
    def _get_docs(self, path: str) -> List[Doc]:
        ...
        return self._tokenize_docs(path)
