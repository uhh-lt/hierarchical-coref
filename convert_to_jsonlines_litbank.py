...
def build_one_jsonline(filename: str,
    ...
    with open(filename, mode="r", encoding="utf8") as f:
        ...
        # # print(f'len(parsed_sents) : {len(parsed_sents)}')
        # if(len(sents) != len(parsed_sents)):
        #     # for x in sents:
        #     #     print(x)
    ...
    # to_be_removed = []   # for sentences containing hyphenated tokens
    # for sent_id, sources in enumerate(zip(sents, parsed_sents)):
    #     # each sent is a token of a sentence
    #     sent, parsed_sent = [s.splitlines() for s in sources]
    #     # print(f'len(sent) : {len(sent)}')
    #     # print(f'len(parsed_sent) : {len(parsed_sent)}')
    #     if( len(sent) != len(parsed_sent)):
    ...
    #         # sents.pop(sent_id)
    #         sents[sent_id] = -1
    #         parsed_sents[sent_id] = -1
    #         # parsed_sents.pop(sent_id)
    #         # print(f'removed')
    
    # sents = [x for x in sents if x!=-1]
    # parsed_sents = [x for x in parsed_sents if x!=-1]
...
def extract_trees_from_file(fileobj: TextIO) -> Generator[str, None, None]:
    # Convert conll to text file
    print(fileobj.name)
    txt_file_path = os.path.join(fileobj.name[:-14] + ".txt")
    # txt_file_path = os.path.join(conll_file[:-11] + ".txt")
    ...
    # Run Stanford Parser command
    parser_dir = "downloads/stanford-parser-full-2020-11-17"  # Replace with actual path
    parser_cmd = f"java -cp '{parser_dir}/*' edu.stanford.nlp.parser.lexparser.LexicalizedParser -tokenized -sentences newline -outputFormat 'penn' edu/stanford/nlp/models/lexparser/englishPCFG.ser.gz {txt_file_path}"
    process = subprocess.Popen(parser_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    if process.returncode == 0:
        ...
        print(f"An error occurred while running the Stanford Parser: {stderr.decode('utf-8')}")
        return ""
        ...
        for filename in filelist:
            with open(filename, mode="r", encoding="utf8") as f_to_read:
                ...
                with open(temp_path, mode="w", encoding="utf8") as f_to_write:
                    f_to_write.write(extract_trees_from_file(f_to_read))
